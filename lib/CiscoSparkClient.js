...
CiscoSparkClient.prototype._getFilename = function(url,callback)
{
    var self = this
    ...
    function (error, response, body) {
        if(error) {
            ...
            try {
                lookupName = self._parseFileName(response.headers['content-disposition'])
                if (lookupName) {
                    callback(null,lookupName,response.body)
                }
                else {
                    callback(e)
                }
...
CiscoSparkClient.prototype.getRoom = function(roomId, callback){
    var self = this
    var url = this.apiURL+"rooms/"+roomId+"?showSipAddress=true"
...
CiscoSparkClient.prototype.getFilename = function(fileURL, callback){
    var self = this
    var url = fileURL

    if(fileURL && callback){
        self._getFilename(url,callback)
    }
    else
    {
        if (callback)
